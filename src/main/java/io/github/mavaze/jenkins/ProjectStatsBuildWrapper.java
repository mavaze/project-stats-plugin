package io.github.mavaze.jenkins;
...
public class ProjectStatsBuildWrapper extends BuildWrapper {
    ...
    private static String generateReport(String projectName, ProjectStats stats) throws IOException {
        ...
        try (
                ...
                InputStream in = ProjectStatsBuildWrapper.class.getResourceAsStream(REPORT_TEMPLATE_PATH)) {
            byte[] buffer = new byte[1024];
            int read;
            while ((read = in.read(buffer)) >= 0) {
                bOut.write(buffer, 0, read);
            }
            content = new String(bOut.toByteArray(), UTF_8);
        }
    ...
    private ProjectStats buildStats(FilePath workspace) throws IOException, InterruptedException {
        int classesNumber = 0;
        int linesNumber = 0;
        Stack<FilePath> toProcess = new Stack<>();
        toProcess.push(workspace);
